...
def generate_alt_examples(name: str, description: str, templates: List[str], p2d: bool) -> List[Dict[str, str]]:
    ...
    How this works: For each template, we generate a prompt that asks gpt-3.5-turbo-instruct to modify the description to fit the template. We then fill the template with the name and the description.
    ...
    model = OpenAIAPI(model_name="gpt-3.5-turbo-instruct", max_parallel=len(prompts))
